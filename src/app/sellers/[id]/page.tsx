...
export default function SellerProfilePage({ params }: { params: { id: string } }) {
  ...
  const handleVerificationSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error || 'Verification submission failed.')
      }
      ...
      setMessage('Documents uploaded. Your verification request is now pending review.')
      event.currentTarget.reset()
    } catch (submitError) {
      console.error('Error submitting verification documents:', submitError)
      setError(submitError instanceof Error ? submitError.message : 'Unable to submit verification documents.')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <main className="min-h-screen bg-white">
      ...
      <section className="container section pt-32 md:pt-40 pb-14">
        {loading ? (
          <div className="card p-10 text-center text-gray-500">Loading seller profile...</div>
        ) : !profile ? (
          <div className="card p-10 text-center text-gray-500">Seller profile not found.</div>
        ) : (
          <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr]">
            <div>
              <p className="badge mb-6">Seller Profile</p>
            ...
            <div className="card p-6">
              <div className="mb-4 flex items-center gap-3">
                ...
                <h2 className="text-2xl font-bold text-gray-950">Verification status</h2>
              </div>
              <p className="mb-4 text-sm leading-relaxed text-gray-600">
                Verified sellers receive a green checkmark badge on their profile and listings. Uploaded documents are stored privately in Supabase Storage.
              </p>
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 text-sm text-gray-600">
                <p><strong>Status:</strong> {profile.verification_status}</p>
                <p><strong>Email:</strong> {profile.email}</p>
                {profile.verification_submitted_at && (
                  <p><strong>Last submitted:</strong> {new Date(profile.verification_submitted_at).toLocaleString()}</p>
                )}
              </div>
            </div>
          </div>
        ...
        <>
          <section className="container pb-16">
            <div className="mb-8 flex items-center gap-3">
              ...
              <h2 className="text-3xl font-bold text-gray-950">Active Listings</h2>
            </div>
          ...
          <section className="bg-gray-50 py-20">
            <div className="container grid gap-10 lg:grid-cols-[1fr_1.1fr]">
              <div>
                <p className="badge mb-6">Get Verified</p>
                <h2 className="mb-4 text-4xl font-bold text-gray-950">Submit your verification documents</h2>
                <p className="text-lg leading-relaxed text-gray-600">
                  Upload a government ID, proof of business ownership, and optionally a business license. Once submitted, your profile moves to pending review in the admin panel.
                </p>
              </div>

              <form onSubmit={handleVerificationSubmit} className="card space-y-5 p-8">
                <div>
                  <label className="mb-2 block text-sm font-semibold text-gray-700">Government ID</label>
                  <input type="file" name="governmentId" accept="image/*,.pdf" required />
                </div>

                <div>
                  <label className="mb-2 block text-sm font-semibold text-gray-700">Proof of Business Ownership</label>
                  <input type="file" name="ownershipProof" accept="image/*,.pdf" required />
                </div>

                <div>
                  <label className="mb-2 block text-sm font-semibold text-gray-700">Business License (optional)</label>
