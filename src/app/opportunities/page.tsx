  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        if (!res.ok) {
          throw new Error(
            (payload as OpportunitiesResponse).error || 'Something went wrong.',
          )
        }
    ...
    <main className="min-h-screen bg-gray-50">
      ...
      <section className="px-4 py-12 md:py-16">
        <div className="mx-auto max-w-2xl space-y-10">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Opportunity Zone Finder</h1>
            <p className="mt-1 text-gray-500">
              Tell us about your concept and we&apos;ll surface the strongest neighborhoods for your launch.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5 rounded-xl bg-white p-6 shadow-sm">
            <div>
              ...
              <input
                ...
                placeholder="e.g. Matcha cafe, ramen bar, bubble tea shop"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">Monthly Budget Min ($)</label>
                <input
                  type="number"
                  value={budgetMin}
              ...
              <div>
                <label className="mb-1 block text-sm font-medium text-gray-700">Monthly Budget Max ($)</label>
                <input
                  type="number"
                  value={budgetMax}
            ...
            >
              {streaming ? 'Analyzing target neighborhoods...' : 'Find My Opportunities'}
            </button>
          </form>

          {streaming && zones.length === 0 && (
            <div className="animate-pulse text-center text-sm text-gray-400">
              Claude is analyzing foot traffic, rent trends, and competitive gaps...
            </div>
          )}
          ...
          {zones.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-lg font-semibold text-gray-800">Top 3 Opportunity Zones</h2>
              {zones.map((zone, i) => (
