...
function MapPlaceholder({ titleText, subtitleText }: { titleText: string; subtitleText: string }) {
  return (
    <div className="h-96 bg-gray-100 rounded-lg flex items-center justify-center border-2 border-gray-200">
      <div className="text-center">
        <MapPin className="w-12 h-12 text-accent mx-auto mb-4" />
        <div className="text-gray-600 font-medium">{titleText}</div>
        <div className="text-sm text-gray-500">{subtitleText}</div>
      </div>
    </div>
  )
}
...
}) => <MapPlaceholder titleText={props.placeholderTitle} subtitleText={props.placeholderSubtitle} />
...
type HurdleEntry = { task: string; level: string; description: string }
type TimelineEntry = { phase: string; timeline: string; description: string }

export default function DataPage() {
  const t = useTranslations('data')
  const locale = useLocale() as AppLocale
  const hurdleColorMap: Record<string, string> = {
    'High': 'bg-red-500',
    'Very High': 'bg-red-700',
    'Medium': 'bg-yellow-500',
  }
  const timelineColorMap = ['bg-blue-500', 'bg-purple-500', 'bg-green-500', 'bg-orange-500', 'bg-teal-500']
  const hurdles = t.raw('hurdles') as HurdleEntry[]
  const timelines = t.raw('timelines') as TimelineEntry[]
  const labelForLevel = (level: string) => {
    try {
      return t(`levelLabels.${level}` as never)
    } catch {
      return level
    }
  }
      ...
      <section className="container section pt-32 md:pt-40 pb-20">
        <div className="text-center max-w-4xl mx-auto">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            ...
            <div className="badge">{t('badgeInsights')}</div>
          </div>
        </div>
      </section>
          ...
          <div className="bg-white p-6 border-2 border-gray-200 rounded-2xl shadow-lg mb-12">
            <div className="h-[500px] rounded-xl overflow-hidden border border-gray-100">
              <MapComponent
                ...
                placeholderTitle={t('mapPlaceholderTitle')}
                placeholderSubtitle={t('mapPlaceholderSubtitle')}
              />
            </div>
          </div>
                ...
                <div className="p-8">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
                    <div className="text-center bg-gradient-to-br from-accent/5 to-accent/10 p-6 rounded-xl border border-accent/20">
                      ...
                      <div className="text-sm text-gray-700 font-medium">{t('metricAvgRent')}</div>
                      <div className="text-xs text-gray-500 mt-1">{t('metricCommercialSpace')}</div>
                    </div>
                    ...
                    <div className="text-center bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-xl border border-blue-200">
                      ...
                      <div className="text-sm text-gray-700 font-medium">{t('metricFootTraffic')}</div>
                      <div className="text-xs text-gray-500 mt-1">{t('metricDailyFlow')}</div>
                    </div>
                    ...
                    <div className="text-center bg-gradient-to-br from-green-50 to-green-100 p-6 rounded-xl border border-green-200">
                      ...
                      <div className="text-sm text-gray-700 font-medium">{t('metricAsianDemand')}</div>
                      <div className="text-xs text-gray-500 mt-1">{t('metricMarketSaturation')}</div>
                    </div>
                    ...
                    <div className="text-center bg-gradient-to-br from-orange-50 to-orange-100 p-6 rounded-xl border border-orange-200">
                      ...
                      <div className="text-sm text-gray-700 font-medium">{t('metricCompetitors')}</div>
                      <div className="text-xs text-gray-500 mt-1">{t('metricAsianRestaurants')}</div>
                    </div>
                  </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div>
                        ...
                        <div className="text-sm text-gray-700">
                          {selectedNeighborhood.foot_traffic_score > 80 ? t('trafficHigh') :
                           selectedNeighborhood.foot_traffic_score > 60 ? t('trafficMedium') :
                           t('trafficLow')}
                        </div>
                      </div>
                      <div>
                        <h5 className="font-semibold text-gray-900 mb-3 flex items-center">
                          ...
                          {t('competitionAssessment')}
                        </h5>
                        <div className="text-sm text-gray-700">
                          {selectedNeighborhood.asian_dining_score > 80 ? t('competitionHigh') :
                           selectedNeighborhood.asian_dining_score > 60 ? t('competitionMedium') :
                           t('competitionLow')}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex justify-end space-x-4">
                    ...
                    <button
                      onClick={() => window.open(localizePath('/waitlist', locale), '_blank')}
            ...
            <div id="detailed-analysis" className="bg-gradient-to-r from-accent/5 to-accent/10 p-8 border-2 border-accent/20 rounded-2xl shadow-lg">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-8">
                <div className="text-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                  ...
                  <div className="text-sm text-gray-600 font-medium">{t('metricAvgRent')}</div>
                  <div className="text-xs text-gray-500 mt-1">{t('metricCommercialSpace')}</div>
                </div>
                ...
                <div className="text-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                  ...
                  <div className="text-sm text-gray-600 font-medium">{t('metricFootTraffic')}</div>
                  <div className="text-xs text-gray-500 mt-1">{t('metricDailyFlow')}</div>
                </div>
                ...
                <div className="text-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                  ...
                  <div className="text-sm text-gray-600 font-medium">{t('metricAsianDemand')}</div>
                  <div className="text-xs text-gray-500 mt-1">{t('metricMarketSaturation')}</div>
                </div>
                ...
                <div className="text-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                  ...
                  <div className="text-sm text-gray-600 font-medium">{t('metricCompetitors')}</div>
                  <div className="text-xs text-gray-500 mt-1">{t('metricAsianRestaurants')}</div>
                </div>
              </div>
              ...
              <div className="bg-white p-6 rounded-xl border border-gray-100">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <h5 className="font-semibold text-gray-900 mb-3">{t('trafficAnalysis')}</h5>
                    <ul className="text-sm text-gray-700 space-y-2">
                      <li className="flex items-start">
                        ...
                        {selectedNeighborhood.foot_traffic_score > 80 ? t('trafficHighPlain') : selectedNeighborhood.foot_traffic_score > 60 ? t('trafficMediumPlain') : t('trafficLowPlain')}
                      </li>
                    </ul>
                  </div>
                  <div>
                    <h5 className="font-semibold text-gray-900 mb-3">{t('competitionAssessment')}</h5>
                    <ul className="text-sm text-gray-700 space-y-2">
                      <li className="flex items-start">
                        ...
                        {selectedNeighborhood.asian_dining_score > 80 ? t('competitionHighPlain') : selectedNeighborhood.asian_dining_score > 60 ? t('competitionMediumPlain') : t('competitionLowPlain')}
                      </li>
                    </ul>
                  </div>
                </div>
            ...
            {loading ? (
              <div className="flex items-center justify-center h-96">
                <div className="animate-pulse">
                  <div className="text-gray-500 mb-4">{t('loadingMarketData')}</div>
              ...
              <div className="flex items-center justify-center h-96">
                <div className="text-red-500 text-center">
                  <div className="text-lg font-semibold mb-2">{t('loadError')}</div>
              ...
              <>
                <ResponsiveContainer width="100%" height={450}>
                  <BarChart data={rentData} margin={{ top: 20, right: 30, left: 20, bottom: 60 }}>
                    ...
                    <YAxis
                      stroke="#666"
                      fontSize={12}
                      label={{ value: t('rentChartYAxis'), angle: -90, position: 'insideLeft', style: { textAnchor: 'middle' } }}
                    />
                    <Tooltip
                      formatter={(value) => [`$${value}`, t('rentChartTooltip')]}
                ...
                <div className="mt-6 p-4 bg-accent/5 rounded-lg border border-accent/20">
                  <p className="text-sm text-gray-700 text-center">
                    <strong>{t('dataSourceLabel')}</strong> {t('dataSourceBody')}
                  </p>
                </div>
              </>
            )}
        ...
        <div className="overflow-x-auto border-2 border-gray-200 rounded-2xl shadow-lg">
          {loading ? (
            <div className="flex items-center justify-center h-48">
              <div className="animate-pulse">
                <div className="text-gray-500 mb-4">{t('loadingNeighborhoodData')}</div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gradient-to-r from-accent/10 to-accent/5 border-b-2 border-accent/20">
                  <th className="px-8 py-6 text-left font-serif font-bold text-gray-900 text-lg">{t('colNeighborhood')}</th>
                  <th className="px-8 py-6 text-left font-serif font-bold text-gray-900 text-lg">{t('colRent')}</th>
                  <th className="px-8 py-6 text-center font-serif font-bold text-gray-900 text-lg">{t('colTraffic')}</th>
                  <th className="px-8 py-6 text-center font-serif font-bold text-gray-900 text-lg">{t('colDemand')}</th>
                  <th className="px-8 py-6 text-center font-serif font-bold text-gray-900 text-lg">{t('colCompetitors')}</th>
                </tr>
              </thead>
              <tbody>
                {neighborhoodMetrics.map((neighborhood, idx) => (
                  <tr key={idx} className="border-b border-gray-100 hover:bg-accent/5 transition-colors duration-150">
                    ...
                    <td className="px-8 py-6 text-accent font-bold text-lg">{neighborhood.rent}{t('perSqFt')}</td>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="bg-white p-8 border-2 border-gray-200 rounded-2xl shadow-lg">
              ...
              <div className="space-y-6">
                {hurdles.map((item, idx) => (
                  <div key={idx} className="flex items-start gap-4 pb-6 border-b border-gray-100 last:border-0">
                    <div className={`w-4 h-4 rounded-full ${hurdleColorMap[item.level] || 'bg-yellow-500'} mt-1 flex-shrink-0`}></div>
            ...
            <div className="bg-white p-8 border-2 border-gray-200 rounded-2xl shadow-lg">
              ...
              <div className="space-y-6">
                {timelines.map((item, idx) => (
                  <div key={idx} className="flex items-start gap-4 pb-6 border-b border-gray-100 last:border-0">
                    <div className={`w-4 h-4 rounded-full ${timelineColorMap[idx % timelineColorMap.length]} mt-1 flex-shrink-0`}></div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2">
